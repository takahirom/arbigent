...
A log records which kind of device it was recorded on and replays only on that kind. Replaying is driven through the same device layer a scenario run uses, so an Android log needs an Android device and an iOS log an iOS one. The recording is not tied to one platform: it listens to the commands the device layer sends, so it works wherever that layer drives a device. Android and iOS have been run end to end; Web should work the same way but has not been exercised yet. Two commands are the exception there: the web driver navigates browser history for a back press and scrolls the document with JavaScript, neither of which a key press or a swipe stands in for, so both are recorded as `unsupported` on Web rather than as an interaction that would land somewhere else.
...
- `--no-wait` skips the readiness waits: the wait for the step's target, the wait for the screen hints and the wait for the end screen. A `wait` the recording itself sent is still sent, because it is part of the interaction rather than a guess about readiness, and the end screen is still checked - just immediately, without waiting for it to arrive. Useful when the waiting is what is going wrong.
...
Before each step, the replay waits for the element the step acted on (`target`) to appear, matching text, resource id and accessibility id, and goes ahead the moment it is there. A step that pressed a bare key with no target, such as a "next" on a splash screen, instead waits for any of the `screen` hints recorded for it: up to five identities of elements the AI saw on that screen. A step with neither -- what an opaque screen such as a canvas or a video surface records -- has nothing to wait *for*, so the replay waits the budget out instead of skipping it: the recorded gap is still the time the app was given before that action. How long it waits comes from the gap the recording itself had between the steps, never below 10 seconds nor above a minute, because a short recorded gap says the recording run was fast, not that the replayed screen will be — and a wait that ends too early here ends the whole replay, since there is no AI to carry on from the screen it stopped on.
...
- `init`: an event sent during the task's setup phase (`launch_app` with `launchArguments`, and the `permissions` and `clearKeychain` the launch asked for, `clear_state`). `permissions` and `clearKeychain` are absent when the recorded launch set neither, so a replay leaves Maestro to fill in its own defaults exactly as the recording did rather than freezing today's default into the log.
