...
Before each step, the replay waits for the element the step acted on (`target`) to appear, matching text, resource id and accessibility id, and goes ahead the moment it is there. A step that pressed a bare key with no target, such as a "next" on a splash screen, instead waits for any of the `screen` hints recorded for it: up to five identities of elements the AI saw on that screen. How long it waits comes from the gap the recording itself had between the steps, never below 10 seconds nor above a minute, because a short recorded gap says the recording run was fast, not that the replayed screen will be — and a wait that ends too early here ends the whole replay, since there is no AI to carry on from the screen it stopped on. A step with neither — what an opaque screen such as a canvas or a video surface records — has nothing to wait *for*, so the replay waits that budget out instead of skipping the step: the recorded gap is still the time the app was given before that action.
