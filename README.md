...
*   **Speed (1/5):** Arbigent's speed is currently limited by the underlying AI technology and the need to interact with the application's UI in real-time. This makes it slower than traditional unit or integration tests.  
    * We have introduced some mechanisms to address this:
       ...
       *  **Optimistic trace replay** (`optimisticTraceReplay`) replays the actions a scenario recorded on its last success without asking the AI for each step. Unlike AI result caching it tolerates a changed UI tree, because it re-locates each target element by its text/ID. If an element is gone or an assertion fails, the scenario is retried in the normal AI-driven mode.  
...
*   **Utilization (1/5):** Arbigent requires both device resources (emulators or physical devices) and AI resources, which can be costly. (AI cost can be around $0.005 per step and $0.02 per task when using GPT-4o.)
    *   **Optimistic trace replay** removes the per-step AI cost for a scenario that already passed, leaving only the image assertions to pay for.
