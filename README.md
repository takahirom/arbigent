...
*   **Utilization (1/5):** Arbigent requires both device resources (emulators or physical devices) and AI resources, which can be costly. (AI cost can be around $0.005 per step and $0.02 per task when using GPT-4o.)
    ...
    *   **Replay scripts** (`settings.replayScripts`) write down what a successful scenario sent to the device as a jsonl log and a readable summary, so `arbigent replay` can get a coding agent or a person back to the same screen later without an AI run at all. See [arbigent-replay-scripts.md](arbigent-replay-scripts.md).
...
HTTPS and a redirect that leaves HTTPS is refused; the one exception is a mirror on the
loopback interface (`localhost`, `127.0.0.1`, `::1`), which may be plain HTTP because
there is nothing in between. The pinning step therefore needs `curl`. Pass
`--sha256 <digest>` to `arbigent wrapper` to supply the digest yourself.
