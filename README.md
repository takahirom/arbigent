...
*   **Utilization (1/5):** Arbigent requires both device resources (emulators or physical devices) and AI resources, which can be costly. (AI cost can be around $0.005 per step and $0.02 per task when using GPT-4o.)
    ...
    *   **Replay scripts** (`settings.replayScripts`) write down what a successful scenario sent to the device as a jsonl log, a readable summary and an `adb`-only runner, so a coding agent or a person can get back to the same screen later without an AI run at all. See [arbigent-replay-scripts.md](arbigent-replay-scripts.md).
