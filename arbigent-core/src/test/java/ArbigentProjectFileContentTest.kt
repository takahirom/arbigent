...
""".trimEnd(),
  ...
  private val projectWithReconnect = ArbigentProjectSerializer().load(
    """scenarios:
- id: "reconnect-test-id"
  goal: "Test reconnect initialization method"
  initializationMethods:
  - type: "Reconnect"
  - type: "LaunchApp"
...
"""
  ...
  @Test
  fun testReconnectInitializationMethod() {
    assertEquals(1, projectWithReconnect.scenarioContents.size)
    
    val scenario = projectWithReconnect.scenarioContents[0]
    assertEquals("reconnect-test-id", scenario.id)
    assertEquals("Test reconnect initialization method", scenario.goal)
    assertEquals(2, scenario.initializationMethods.size)
    
    // Test first initialization method (Reconnect)
    val reconnectMethod = scenario.initializationMethods[0]
    assertTrue(reconnectMethod is ArbigentScenarioContent.InitializationMethod.Reconnect, 
      "First method should be Reconnect")
    
    // Test second initialization method (LaunchApp)
    val launchAppMethod = scenario.initializationMethods[1]
    assertTrue(launchAppMethod is ArbigentScenarioContent.InitializationMethod.LaunchApp, 
      "Second method should be LaunchApp")
    assertEquals("com.example.reconnect", 
      (launchAppMethod as ArbigentScenarioContent.InitializationMethod.LaunchApp).packageName,
      "Package name should match")
  }

  @Test
  fun testReconnectIsObjectType() {
    val projectWithReconnect = ArbigentProjectSerializer().load(
      """scenarios:
- id: "reconnect-test"
  goal: "Test reconnect"
  initializationMethods:
...
"""
    ...
    val reconnectMethod = projectWithReconnect.scenarioContents[0].initializationMethods[0]
    assertTrue(reconnectMethod is ArbigentScenarioContent.InitializationMethod.Reconnect)
    // Reconnect is now an object, not a data class with parameters
    assertEquals(ArbigentScenarioContent.InitializationMethod.Reconnect, reconnectMethod)
  }
}
